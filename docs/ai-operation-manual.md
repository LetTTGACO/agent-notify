...
### Prepare the Provider app (do this before Step 2.2)

Once the Provider is chosen, the user must install its client app on the receiving device and obtain the endpoint — otherwise Step 2.2 has nothing to paste into `.env`. Tell the user only what to do; the real endpoint value stays in the app/file and never enters the conversation.

**Bark** (iPhone / Apple Watch):

- Install the Bark app from the App Store on the receiving device.
- Open Bark; it shows a device-specific URL like `https://api.day.app/your-device-key`. This whole URL is the `BARK_ENDPOINT` the user will paste into `.env` in Step 2.2.
...
| `uname -s` output | OS | Config directory base |
| --- | --- | --- |
...
Every "copy to where" / "where does the config file go" below is resolved via this table. A full three-platform path comparison appears inside each step.
...
### 2.2 Guide the user to edit `.env` by hand (sensitive — never in conversation)
...
`doctor` should show several `OK` lines (endpoint configured, log dir writable, `/health` reachable). If it complains about a missing `AGENT_NOTIFY_TOKENS` or endpoint, the user has not finished editing `.env` — go back to 2.2 and have them continue; do not fill it for them.

The server is ready. Keep `pnpm dev` running and proceed to Step 3.

### 2.4 (Optional) Docker deployment
...
## Step 3 — Deploy the plugin/adapter side
...
Note this output — it is the path used in Step 4's hooks. → Go to Step 4.
...
## Step 4 — Configure agent hooks

OpenCode skips this step. Claude Code and Codex need hooks merged into their respective config files. Below, replace `/ABS/PATH/...` with the absolute path from Step 3's `printf` output.
...
What each hook does: `UserPromptSubmit` records this turn's start time (no notification); `Notification` pushes permission/MCP-interaction notifications; `Stop` pushes a completion notification once the completion threshold (default 120s) is met; `StopFailure` pushes a failure notification. `Notification` needs no matcher — the adapter and server ignore `idle_prompt` by default.
...
- `pnpm agent-notify doctor` reports a missing endpoint/token → the user's `.env` is not finished; back to Step 2.2.
...
### 5.5 Send a test notification (optional)
