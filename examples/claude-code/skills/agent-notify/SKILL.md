---
name: agent-notify
description: "Handle AgentNotify Claude Code control commands. Use when the user invokes /agent-notify with no arguments, on, off, off duration, off persist, or status. Explain the local notification switch result and read the Claude Code AgentNotify state file for status."
---

# AgentNotify Claude Code Command

You are handling an AgentNotify notification switch command inside Claude Code.

The Claude Code AgentNotify hook adapter handles the local side effects when the
prompt is submitted:

- It writes `~/.config/agent-notify/state/claude-code.json`.
...
For `off`, `off <duration>`, `off persist`, and `on`, trust that the Claude Code
hook adapter has already applied the state change. Reply with exactly one short
confirmation sentence.

Use these templates:

- `off`: `AgentNotify: Claude Code 当前会话通知已关闭。`
- `off <duration>`: `AgentNotify: Claude Code 通知已关闭，持续 <duration>。`
- `off persist`: `AgentNotify: Claude Code 通知已持久关闭。`
- `on`: `AgentNotify: Claude Code 通知已开启。`
...
read and that the Claude Code adapter treats bad state as enabled.
...
The skill usually cannot know the current Claude Code session id. Do not claim
that the current session is muted unless the current session id is explicitly
...
- On: `AgentNotify: Claude Code 通知已开启。`
- Persistent mute: `AgentNotify: Claude Code 通知已持久关闭。`
- Timed mute: `AgentNotify: Claude Code 通知已关闭，直到 <ISO timestamp>。`
- Session records only: `AgentNotify: Claude Code 有 <count> 个会话静音记录；当前会话是否静音无法从 skill 上下文确认。`
- Bad state: `AgentNotify: Claude Code 状态文件无法读取，adapter 会按通知已开启处理。`
